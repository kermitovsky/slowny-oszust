...
<body>
  ...
  <div id="votingOverlay" class="cinematic-overlay voting-style">
    <h1 class="cinematic-title">GŁOSOWANIE!</h1>
  ...
  <div id="loginScreen" class="container" data-screen>
    ...
    <input type="text" id="playerName" placeholder="Twój Nick" maxlength="15">
    <div id="emojiSelection"></div>
    <button id="createRoom">Stwórz Pokój</button>
    <input type="text" id="roomCodeInput" placeholder="Kod pokoju" maxlength="4" style="text-transform: uppercase;">
  ...
  <div id="gameScreen" class="container" data-screen>
    ...
    <div id="roomCodeContainer">
      ...
      <button id="copyRoomCode" title="Skopiuj kod">📋</button>
      <button id="roomSettingsBtn" title="Ustawienia Pokoju" style="display: none;">⚙️</button>
    </div>
    ...
    <button id="startGame" style="display: none;">Rozpocznij Grę</button>
    <button id="startVoteBtn" style="display: none;">Rozpocznij Głosowanie</button>
    <button id="confirmVoteBtn" style="display: none;">Zatwierdź Głos</button>
    <button id="endRound" style="display: none;">Zakończ Rundy Wcześnie</button>
  ...
  <div id="categorySelectionBox" class="modal-box">
    <button class="close-btn" id="closeCategories">×</button>
    <p>Wybierz kategorie słów</p>
    <div class="category-grid custom-scrollbar">
      <button class="category-btn custom-new-btn" id="createCustomCategoryBtn">+ Stwórz własną</button>
    </div>
    <div class="checkbox-container" id="allCategoriesBtn">
      <input type="checkbox" id="allCategoriesCheckbox">
      <label for="allCategoriesCheckbox"><div class="checkbox"></div>Wybierz wszystkie</label>
    </div>
    <button id="confirmCategories">Dalej ➔</button>
  </div>

  <div id="customCategoryBox" class="modal-box">
    <button class="close-btn" id="closeCustomCategoryBtn">×</button>
    <h2>Własna Kategoria</h2>
    <input type="text" id="customCategoryNameInput" placeholder="Nazwa (np. Hasła z naszej pracy)" maxlength="20">
    <div class="custom-word-adder">
      <input type="text" id="customWordInput" placeholder="Dodaj słowo..." maxlength="30">
      <button id="addCustomWordBtn">+</button>
    </div>
    <ul id="customWordsList" class="custom-scrollbar"></ul>
    <button id="saveCustomCategoryBtn" disabled>Zapisz i użyj</button>
  </div>
  ...
  <div id="impostorSelectionBox" class="modal-box">
    ...
    <div id="loadingMessage" style="display:none; color: var(--c-accent);">Ładowanie słów...</div>
    <div id="recommendedPlayers">Zalecana liczba graczy: 3-5</div>
    <button id="confirmImpostors">Dalej ➔</button>
  </div>

  <div id="impostorHintBox" class="modal-box">
    <p>Podpowiedź dla oszusta</p>
    ...
    <div class="checkbox-container" id="customHintToggleContainer" style="margin-bottom: 2rem; border: 2px solid var(--c-primary); padding: 1rem; border-radius: 1rem; background: rgba(142,68,173,0.1);">
      ...
      <label for="customHintCheckbox">
        ...
        <div style="text-align: left; line-height: 1.2;">
          <span style="font-weight: bold; color: var(--c-text-light);">Własne podpowiedzi graczy</span><br>
          <span style="font-size: 1.2rem; opacity: 0.8;">Oszust dostanie jedno słowo od niewinnego</span>
        </div>
      </label>
    </div>
  ...
  <div id="impostorTeamBox" class="modal-box">
    <p>Czy oszuści się znają?</p>
    <div class="team-selector">
      <button id="teamKnowsBtn">Tak</button>
      <button id="teamNotKnowsBtn" class="selected">Nie</button>
    </div>
    <button id="confirmTeamSettingsBtn">Stwórz Pokój ✔</button>
  </div>

  <div id="roomSettingsBox" class="modal-box">
    <button class="close-btn" id="closeRoomSettingsBtn">×</button>
      ...
      <div class="settings-group">
        ...
        <div class="checkbox-container" id="setCustomHintToggleContainer" style="margin: 1rem auto; padding-bottom: 1rem; border-bottom: 1px solid rgba(255,255,255,0.2);">
          ...
          <label for="setCustomHintCheckbox">
            ...
            <div style="text-align: left; line-height: 1.2;">
              <span style="font-weight: bold; color: var(--c-text-light);">Własne podpowiedzi graczy</span><br>
              <span style="font-size: 1.1rem; opacity: 0.8;">Wszyscy piszą słowo przed rundą</span>
            </div>
          </label>
        </div>
  ...
  <div id="hintInputBox" class="modal-box">
    <h2 id="hintInputTitle" style="font-size: 3rem; margin-bottom: 1rem;">Czas na podpowiedź!</h2>
    <p id="hintInputDesc" style="font-size: 1.6rem; margin-bottom: 2rem; line-height: 1.4;"></p>
    <input type="text" id="customHintInput" placeholder="Wpisz jedno słowo..." maxlength="20" autocomplete="off">
    <button id="submitCustomHintBtn" style="background: var(--c-success);">Gotowe</button>
    <div id="hintWaitingMessage" style="display:none; font-size: 1.8rem; margin-top: 1.5rem; color: var(--c-accent); font-weight: bold; animation: pulse 1.5s infinite;">Oczekiwanie na resztę graczy...</div>
  </div>

  <div id="rulesBox" class="modal-box">
    <button class="close-btn" id="closeRulesTop">×</button>
    <h2>Jak Grać?</h2>
    <div class="custom-scrollbar" style="max-height: 60vh; overflow-y: auto; padding-right: 10px;">
      <p>👥 <b>Gracze:</b> Minimum 3 (Zalecane 4+).</p>
      <h3>Zasady:</h3>
      <ul>
        <li>Oszust (Impostor) <b>nie zna hasła</b>. Reszta graczy je zna.</li>
        <li>Gra losuje, kto zaczyna (🎤). Zaczynająca osoba mówi <b>jedno słowo</b> kojarzące się z hasłem.</li>
        <li>Kolejni gracze (zgodnie z ruchem wskazówek zegara) też podają po jednym słowie.</li>
        <li><b>Cel Niewinnych:</b> Udowodnić, że znają hasło, ale podać słowo na tyle trudne, by oszust się nie domyślił.</li>
        <li><b>Cel Oszusta:</b> Udawać, że wie o co chodzi i nie dać się złapać!</li>
      </ul>
      <h3>Głosowanie:</h3>
      <ul>
        <li>Po ustalonym czasie (lub liczbie okrążeń) rozpoczyna się głosowanie.</li>
        <li>Jeśli większość wskaże oszusta - <b>Niewinni wygrywają</b>.</li>
        <li>Jeśli oszust przetrwa (remis lub wskazano niewinnego) - <b>Oszust wygrywa</b>.</li>
      </ul>
    </div>
    <button class="close-btn-bottom" id="closeRules">Zrozumiałem</button>
  </div>
